...
namespace BabaIsYou.Zip {
	internal class InflaterDynHeader {
		...
		private static readonly int[] MetaCodeLengthIndex = { 16, 17, 18, 0, 8, 7, 9, 6, 10, 5, 11, 4, 12, 3, 13, 2, 14, 1, 15 };
		...
		private IEnumerable<bool> CreateStateMachine() {
			...
			if (litLenCodeCount > LITLEN_MAX) throw new Exception(nameof(litLenCodeCount));
			if (distanceCodeCount > DIST_MAX) throw new Exception(nameof(distanceCodeCount));
			if (metaCodeCount > META_MAX) throw new Exception(nameof(metaCodeCount));
		...
		public InflaterHuffmanTree LiteralLengthTree => litLenTree ?? throw new Exception("Header properties were accessed before header had been successfully read");

		/// <summary>
		/// Get distance huffman tree, must not be used before <see cref="AttemptRead"/> has returned true
		/// </summary>
		/// <exception cref="StreamDecodingException">If hader has not been successfully read by the state machine</exception>
		public InflaterHuffmanTree DistanceTree => distTree ?? throw new Exception("Header properties were accessed before header had been successfully read");
